...
# שינוי: שני משתנים נפרדים
API_SECTION_NAME = "commentisfree" # <-- שם המדור בבקשת ה-API
LOCAL_FOLDER_NAME = "opinion"     # <-- שם התיקייה שבה נשמור


def save_article(article):
    """שומר כתבה אחת לקובץ טקסט בתיקיית opinion."""
    ...
    # בניית הכותרת לקובץ
    section = article.get("sectionName", LOCAL_FOLDER_NAME)
    ...
    # שמירה ל-data/opinion/<article_id>.txt (משתמש בשם התיקייה המקומי)
    out_dir = Path("data") / LOCAL_FOLDER_NAME
...
def main():
    print(f"🚀 מתחיל לאסוף כתבות עבור הנושא: {LOCAL_FOLDER_NAME.upper()} (API Section: {API_SECTION_NAME})...")
    ...
    for page_num in range(1, total_pages + 1):
        params = {
            "api-key": API_KEY,
            "page-size": PAGE_SIZE,
            "section": API_SECTION_NAME, # <-- משתמש בשם ה-API הנכון
            "order-by": "newest",
    ...
    print(f"✨ סיום איסוף עבור {LOCAL_FOLDER_NAME.upper()}. סה״כ נשמרו: {total_articles_saved} כתבות.")
